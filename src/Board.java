...
class Board {
    ...
    Board(Board board0) {
        ...
        _numPieces[BLUE.ordinal()] = blue;
        _numPieces[RED.ordinal()] = red;
    ...
    boolean legalMove(Move move) {
        if (move == null) {
            ...
            boolean cond1 = _board[index(move.col1(), move.row1())] == EMPTY;
            PieceColor now = _board[index(move.col0(), move.row0())];
            boolean cond2 = now == whoseMove();
            if (cond1 && cond2) {
                return true;
            }
        } else if (move.isPass()) {
    ...
    boolean canMove(PieceColor who) {
        for (char col = 'a'; col < 'h'; col++) {
            for (char row = '1'; row < '8'; row++) {
                if (_board[index(col, row)] == who) {
                    char cStart = (char) (col - 2);
                    for (char col2 = cStart; col2 <= col + 2; col2++) {
                        char rStart = (char) (row - 2);
                        for (char row2 = rStart; row2 <= row + 2; row2++) {
                            if (_board[index(col2, row2)] == EMPTY) {
                                return true;
                            }
                        }
                    }
    ...
    void makeMove(Move move) {
        ...
        _numMove += 1;
        boolean emptyRed = (numPieces(RED) == 0);
        boolean emptyBlue = (numPieces(BLUE) == 0);
        boolean bothNot = !canMove(RED) && !canMove(BLUE);
        if (_numJumps >= JUMP_LIMIT || emptyBlue || emptyRed || bothNot)  {
            if (numPieces(RED) > numPieces(BLUE) || emptyBlue) {
                _winner = RED;
            } else if (numPieces(BLUE) > numPieces(RED) || emptyRed) {
                _winner = BLUE;
            } else {
                _winner = EMPTY;
            }
    ...
    void flip(Move move, PieceColor opponent) {
        char cStart = (char) (move.col1() - 1);
        for (char col = cStart; col < move.col1() + 2; col++) {
            char rStart = (char) (move.row1() - 1);
            for (char row = rStart; row < move.row1() + 2; row++) {
                if (get(col, row) == opponent) {
                    incrPieces(opponent, -1);
                    incrPieces(whoseMove(), 1);
                    set(col, row, whoseMove());
                }
            }
        }
    }
    ...
    void undo() {
        ...
        Integer prev = _undoSquares.pop();
        while (prev != null) {
            incrPieces(get(prev), -1);
            PieceColor last = _undoPieces.pop();
            unrecordedSet(prev, last);
            incrPieces(last, 1);
            prev = _undoSquares.pop();
        }
    ...
    boolean legalBlock(char c, char r) {
        ...
        char refC = (char) ('d' + (char) ('d' - c));
        char refR = (char) ('4' + (char) ('4' - r));
        boolean cond1 = get(c, r) == EMPTY && get(refC, r) == EMPTY;
        boolean cond2 = get(c, refR) == EMPTY && get(refC, refR) == EMPTY;
        return cond1 && cond2 && _numMove == 0;
    ...
    /** Total number of non-undone moves and passes since the
     *  last clear or the beginning of the game. */
