...
class GUI extends TopLevel implements View, CommandSource, Reporter {
    ...
    private synchronized void newGame(String unused) {
        ...
        setAIMode(RED.name(), false);
        setAIMode(BLUE.name(), true);
        send("new");
    ...
    public void announceWin(PieceColor player) {
        if (player == BLUE) {
            showMessage("Blue wins.", "Outcome", "Blue wins.");
        }
        if (player == RED) {
            showMessage("Red wins.", "Outcome", "Red wins.");
        }
        if (player == EMPTY) {
            showMessage("Tie Game.", "Outcome", "Tie Game.");
        }
    }
